...
$(document).ready(function () {
  ...
  if (mapCanvas.length) {
    ...
    for (var i = 0; i < mapCanvas.length; i++) {
      ...
      initLatitude = parseFloat(m.dataset.latitude);
      initLongitude = parseFloat(m.dataset.longitude);
      divId = "#" + m.id;

      // Verifica se as coordenadas são válidas
      if (isNaN(initLatitude) || isNaN(initLongitude)) {
        console.error("Erro: Latitude ou Longitude inválidas para o mapa", m);
        continue;
      }
      ...
      map = new GMaps({
        ...
        scrollwheel: false,
        styles: [
          /* Adicione estilos do mapa aqui, se necessário */
        ]
      });
